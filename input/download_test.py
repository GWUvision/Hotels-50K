...
def resize(imList):
    for im in imList:
        try:
            imUrl = im[2]
            saveDir = os.path.join('./images/test/',im[0])
...
def main():
    jsonDataPath = './input/test_set.json'
    ...
    for hotel in data.keys():
        info = data[hotel]
        for im in info['ims']['tcam'].keys():
            im_path = info['ims']['tcam'][im]['path']
            imList.append((hotel,'tcam',im_path))
